...
const regionOptions = ['전체지역', '전국', '서울', '경기', '인천', '부산', '대구', '광주', '대전', '울산', '세종', '강원', '충북', '충남', '전북', '전남', '경북', '경남', '제주'];
const ageOptions = ['전체연령', '영유아·출산', '아동·청소년', '청년', '중장년', '어르신'];
const targetOptions = ['전체대상', '국가유공자·보훈', '장애인', '소상공인', '농어업인', '저소득층', '신혼부부'];

const ageKeywords = {
  '영유아·출산': ['영유아', '영아', '유아', '신생아', '출산', '출생', '임산부', '임신', '산모', '육아', '보육', '다자녀', '양육'],
  '아동·청소년': ['아동', '어린이', '청소년', '초등', '중등', '고등', '학생', '입학', '학습', '장학', '교복'],
  청년: ['청년', '대학생', '취업준비', '미취업', '구직', '사회초년', '신혼'],
  중장년: ['중장년', '중년', '장년', '신중년', '경력단절', '재취업'],
  어르신: ['어르신', '노인', '고령', '경로', '독거노인', '기초연금', '장수', '65세', '70세'],
};

const targetKeywords = {
  '국가유공자·보훈': ['국가유공자', '보훈', '참전', '독립유공', '상이군경', '고엽제', '보훈대상', '유족', '순직', '참전유공'],
  장애인: ['장애인', '장애아', '장애정도', '발달장애', '중증장애', '장애수당', '장애연금'],
  소상공인: ['소상공인', '자영업', '소기업', '전통시장', '상인', '창업', '폐업', '사업자', '중소기업'],
  농어업인: ['농업', '농가', '농민', '어업', '어민', '임업', '축산', '귀농', '귀어', '농어업', '농촌', '어촌'],
  저소득층: ['저소득', '기초생활', '수급자', '차상위', '한부모', '취약계층', '중위소득', '생계급여', '의료급여'],
  신혼부부: ['신혼', '혼인', '결혼', '예비부부', '부부', '전세자금', '주택자금', '임차보증금'],
};
...
function categoryUrl({ type = params.get('type') || '전체', region = params.get('region') || '전체지역', age = params.get('age') || '전체연령', target = params.get('target') || '전체대상', query = params.get('q') || '' } = {}) {
...
function policySearchText(policy) {
  return [policy.title, policy.institution, policy.summary, policy.target, policy.method, policy.income, policy.region, policy.maxBenefit, (policy.tags || []).join(' ')]
    .join(' ')
    .toLowerCase();
}

function matchingGroups(policy, keywordMap) {
...
function formatDataDate() {
  ...
  return `${new Intl.DateTimeFormat('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit', timeZone: 'Asia/Seoul' }).format(new Date(data.generatedAt))} 기준`;
}
...
function daysLeft(deadline) {
  ...
  if (deadline.length !== 10 || deadline[4] !== '-' || deadline[7] !== '-') return deadline;
...
function policyCard(policy) {
  ...
  return `
    <article class='policy-card' data-type='${type}'>
      <a class='policy-thumb' href='${detailUrl}' aria-label='${title} 상세 보기'>
        <span class='policy-source-mark'>공공</span>
        <small>${type}</small>
      </a>
...
function bindCommonActions() {
  qsa('[data-apply]').forEach((button) => button.addEventListener('click', () => showToast('공식 원문에서 신청 경로를 확인해 주세요.')));
  qsa('[data-store]').forEach((button) => button.addEventListener('click', () => showToast('앱 스토어 연결 준비 중입니다.')));
...
function renderHome() {
  ...
  const target = qs('#featuredPolicies');
  if (target) target.innerHTML = featured.map(policyCard).join('');
}
...
function filteredPolicies() {
  ...
  return policies.filter((policy) => {
    ...
    const ageMatch = age === '전체연령' || matchingGroups(policy, ageKeywords).includes(age);
    const targetMatch = target === '전체대상' || matchingGroups(policy, targetKeywords).includes(target);
...
function renderCategory() {
  ...
  qs('#regionFilter').innerHTML = regionOptions.map((item) => `<a class='${item === region ? 'active' : ''}' href='${categoryUrl({ type, region: item, age, target, query })}'>${escapeHtml(item)}</a>`).join('');
  qs('#ageFilter').innerHTML = ageOptions.map((item) => `<a class='${item === age ? 'active' : ''}' href='${categoryUrl({ type, region, age: item, target, query })}'>${escapeHtml(item)}</a>`).join('');
  qs('#targetFilter').innerHTML = targetOptions.map((item) => `<a class='${item === target ? 'active' : ''}' href='${categoryUrl({ type, region, age, target: item, query })}'>${escapeHtml(item)}</a>`).join('');

  const notice = qs('#resultNotice');
  if (notice) {
    notice.textContent = list.length > visibleList.length ? `${money.format(list.length)}개 중 ${money.format(visibleList.length)}개를 먼저 보여드려요. 지역명이나 대상 키워드를 더 넣으면 정확해집니다.` : '';
  }

  qs('#policyList').innerHTML = visibleList.length ? visibleList.map(policyCard).join('') : `<div class='empty-card'>조건에 맞는 정책이 없습니다. 지역이나 검색어를 넓혀보세요.</div>`;
...
function renderPolicyDetail() {
  ...
  const sourceUrl = safeUrl(policy.sourceUrl);
  const sourceAction = sourceUrl ? `<a class='primary-button' href='${sourceUrl}' target='_blank' rel='noopener'>신청하기</a>` : `<button class='primary-button' type='button' data-apply>지원하기</button>`;
  ...
  qs('#policyDetail').innerHTML = `
    <div class='detail-head'>
      ...
      <div class='detail-actions'>${sourceAction}<a class='ghost-button' href='category.html'>목록 보기</a></div>
    </div>
